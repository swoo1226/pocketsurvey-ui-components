...
export function Index() {
    ...
    return (
        <>
        <h2> 이미지 업로드 대기 상태</h2>
           <ImgVideo qrCode = {null} type="image" mediaSrc={null} onClick={() => {
               setImgSrc(null) 
            }}/>
        <h2>이미지 업로드 완료 후</h2>
            <ImgVideo qrCode = {null} type="image" mediaSrc={imagSrc} onClick={() => {
               setImgSrc(null)
            }}/>
           <h2>영상 업로드</h2>
            <ImgVideo qrCode = {null} mediaSrc={null} type="video" onClick={() => {
               setVideoSrc(null)
            }}/>
        <h2>영상 업로드 후</h2>
            <ImgVideo qrCode = {null} mediaSrc={videoSrc} type="video" onClick={() => {
               setVideoSrc(null)
            }}/>
        </>
    )
