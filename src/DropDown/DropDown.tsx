...
function DropDown({ list, selected, disable, themeColor, onItemClick, className, iconColor, placeholder }: DropDownType): JSX.Element {
    ...
    <DropDownContainer ref={selectionListRef} className={className}>
      ...
      >
        ...
        >
          {selected !== null ? (
            <React.Fragment>
              {list[selected].icon && <Icon color={disable ? "#818282" : iconColor!} icon={list[selected].icon!} width={18} />}
        ...
        <Icon color={disable ? "#818282" : iconColor!} icon="arrow" useCursor={disable ? false : true} width={18} rotate={!isShowList ? 90 : 270} />
      </DropDownBoxContainer>

      <DropDownList isShowList={isShowList} disable={disable} style={!isShowList ? { display: "none" } : { opacity: "1" }} data-testid="dropdownlist-testid">
        {list.map((item, index) => (
          <DropDownItem
            key={index}
            index={index}
            ...
            hidden={item.hidden}
          >
            {item.icon && <Icon color={disable ? "#818282" : iconColor!} icon={item.icon} width={18} />}
